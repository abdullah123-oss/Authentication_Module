// src/pages/DashboardPages/Doctor/DoctorMedicines.jsx

import React, { useState, useMemo } from "react";
import { Link } from "react-router-dom";
import { useQuery } from "@tanstack/react-query";
import { getAllMedicinesForDoctor } from "../../../api/doctorMedicinesApi";

export default function DoctorMedicines() {
  const [search, setSearch] = useState("");
  const [filterCategory, setFilterCategory] = useState("all");

  // Fetch medicines (public)
  const { data, isLoading } = useQuery({
    queryKey: ["doctor-medicines"],
    queryFn: getAllMedicinesForDoctor,
  });
  ...
  return (
    <div className="p-6">
      ...
      {/* Loading */}
      {isLoading && <p className="text-gray-500 text-center">Loading medicines...</p>}
